...
import type { IconCalendarEvent } from '@tabler/icons-react';
import useStyles from './LateralTabs.styles';

interface TabsProps {
  data: {
    demo: React.FC<unknown>;
...
export function LateralTabs({ data }: TabsProps) {
  const [shouldAnimate, setShouldAnimate] = useState(false);
  const { classes, theme } = useStyles({ shouldAnimate });
  const animationTimeout = useRef<number>();
  const [active, setActive] = useState(0);
  const controlSize = useMediaQuery(`(max-width: ${theme.breakpoints.sm}px)`)
    ? 60
    : 80;
  ...
  return (
    <div className={classes.root}>
      <Container fluid>
        <Grid gutter={0} mt="md" justify="center">
          <Grid.Col md={4}>
